...
class MeanAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        neigh_means = tf.divide(tf.reduce_sum(neigh_vecs, axis=1), tf.tile(tf.expand_dims(num_nz+1e-10,-1), [1, feature_dim]))
...
class LRMeanAggregator(Layer):
    ...
    def __init__(self, input_dim, output_dim, neigh_input_dim=None,
        ...
        with tf.variable_scope(self.name + name + '_vars'):
            self.vars['neigh_weights1'] = glorot([neigh_input_dim, np.int(output_dim/3)], name='neigh_weights1')
            
            self.vars['neigh_weights2'] = glorot([np.int(output_dim/3), output_dim], name='neigh_weights2')
            
            self.vars['self_weights1'] = glorot([input_dim, np.int(output_dim/3)], name='self_weights1')
            
            self.vars['self_weights2'] = glorot([np.int(output_dim/3), output_dim], name='self_weights2')
    ...
    def _call(self, inputs):
        ...
        _, num_neigh, feature_dim = neigh_vecs.get_shape().as_list()
        neigh_means = tf.divide(tf.reduce_sum(neigh_vecs, axis=1), tf.tile(tf.expand_dims(num_nz,-1), [1, feature_dim]))
...
class LogicMeanAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        neigh_means = tf.divide(tf.reduce_sum(tf.multiply(neigh_vecs, tf.cast(att, tf.float32)), axis=1), tf.tile(tf.expand_dims(num_nz,-1), [1, feature_dim]))
...
class AttMeanAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        att = tf.reshape(att, [-1, num_neigh, 1])
        att = tf.tile(att, [1, 1, feature_dim])
        neigh_means = tf.divide(tf.reduce_sum(tf.multiply(neigh_vecs,att), axis=1), tf.tile(tf.expand_dims(num_nz,-1), [1, feature_dim]))
...
class GCNAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        _, num_neigh, feature_dim = neigh_vecs.get_shape().as_list()
        means = tf.divide(tf.reduce_sum(tf.concat([neigh_vecs, 
                tf.expand_dims(self_vecs, axis=1)], axis=1), axis=1), tf.tile(tf.expand_dims(1+num_nz+1e-10,-1), [1, feature_dim]))
